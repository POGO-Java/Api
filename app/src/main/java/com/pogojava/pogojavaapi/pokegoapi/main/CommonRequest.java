...
import com.pogojava.pogojavaapi.pokegoapi.exceptions.AsyncRemoteServerException;
...
public class CommonRequest {
	...
	public static GetBuddyWalked.GetBuddyWalkedMessage getDefaultBuddyWalked(PokemonGo api) {
		return GetBuddyWalked.GetBuddyWalkedMessage.newBuilder()
				.build();
	}
	...
	public static GetInventoryMessage getDefaultGetInventoryMessage(PokemonGo api) {
		...
		if (api.getInventories() != null && api.getInventories().getLastInventoryUpdate() != 0) {
	...
	public static InternalServerRequest[] getCommonRequests(PokemonGo api) {
		return new InternalServerRequest[] {
				...
				new InternalServerRequest(RequestType.GET_BUDDY_WALKED,
						CommonRequest.getDefaultBuddyWalked(api))
		};
	}
		...
		try {
			switch (requestType) {
				case CHECK_CHALLENGE:
					CheckChallengeResponseOuterClass.CheckChallengeResponse checkChallengeResponse =
							CheckChallengeResponseOuterClass.CheckChallengeResponse.parseFrom(data);
					if (checkChallengeResponse.getShowChallenge()) {
						api.setInCheckChallend(true);
						if (api.getOnCheckChallengeRequestListener() != null) {
							api.getOnCheckChallengeRequestListener().onCheckChallenge(checkChallengeResponse.getChallengeUrl());
						}
					}
					break;
				case GET_INVENTORY:
