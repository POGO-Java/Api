...
public final strictfp class S2Polyline implements S2Region {
  ...
  public int getNearestEdgeIndex(S2Point point) {
	...
	if (!(numVertices() > 0)) throw new IllegalStateException("Empty polyline");
    if (numVertices() == 1) {
  ...
  public S2Point projectToEdge(S2Point point, int index) {
    ...
    //Preconditions.checkState(numVertices() == 1 || index < numVertices() - 1, "Invalid edge index");
	  if (!(numVertices() > 0)) throw new IllegalStateException("Empty polyline");
	  if (!(numVertices() == 1 || index < numVertices() - 1)) throw new IllegalStateException("Invalid edge index");
    if (numVertices() == 1) {
  ...
  @Override
  public int hashCode() {
    return Utils.hashCode(numVertices, Arrays.deepHashCode(vertices));
  }
}
